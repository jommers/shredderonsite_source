...
class ITSEC_Recaptcha {
	...
	function run() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'itsec_public_scripts' ) );
		add_action( 'login_enqueue_scripts', array( $this, 'login_enqueue_scripts' ) );
		add_action( 'login_enqueue_scripts', array( $this, 'itsec_public_scripts' ) );
	 ...
	 * @return void
	 */
	public static function itsec_public_scripts() {
		...
		wp_register_script( 'google-recaptcha', 'https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit' . $language, array(), '1.0', true );
		wp_enqueue_script( 'google-recaptcha' );

		wp_register_script( 'itsec-recaptcha', $module_path . 'js/itsec-recaptcha.js', array(), $itsec_globals['plugin_build'] );
		wp_enqueue_script( 'itsec-recaptcha' );
		wp_localize_script(
			'itsec-recaptcha',
			'itsec_recaptcha',
			array(
	 ...
	 * @param bool $ajax_load     whether the form is being loaded via AJAX
	 *
	 * @return String the field string
	 */
	public static function show_field( $echo = true, $noscript = true, $margin_top = 0, $margin_right = 0, $margin_bottom = 0, $margin_left = 0, $ajax_load = false ) {
		...
		//Used if loading the form through ajax to ensure the JS is called after page load.
		if ( true === $ajax_load ) {
			$field .= '<script src="https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit" async defer></script>';
		}

		$field .= '<div style="margin: ' . $margin_top . 'px ' . $margin_right . 'px ' . $margin_bottom . 'px ' . $margin_left . 'px;" id="itsec_recaptcha"></div>';
